...
const SECTION_KEYS = ['hero', 'servicesHeading', 'serviceIndividual', 'serviceCouples', 'serviceGroup', 'whyChooseUs'];
...
const HomeDetailsEditor = ({ onCancel, onSave }) => {
    ...
    const fetchSections = async () => {
      try {
        ...
        if (data.success && data.data) {
          ...
          // API returns an object keyed by sectionKey: { hero: {title, body}, ... }
        ...
        showToast(t('homeDetails.toast.loadFailed'), 'error');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      // Build array of sections to save
      ...
      const response = await authenticatedFetch(`${API}/homecontent/bulk`, {
        method: 'PUT',
      ...
      if (data.success) {
        showToast(t('homeDetails.toast.saved'), 'success');
        if (onSave) onSave();
      } else {
        showToast(t('homeDetails.toast.failed'), 'error');
      }
    } catch (error) {
      console.error('Error saving home sections:', error);
      showToast(t('homeDetails.toast.error'), 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div style={{ padding: '1rem' }}>
      ...
      <div style={{
        ...
        <span dangerouslySetInnerHTML={{ __html: t('homeDetails.helpText') }} />
      </div>
        ...
        {SECTION_KEYS.map(sectionKey => (
          ...
          >
            <h3 style={{ 
              ...
              {t(`homeDetails.sections.${sectionKey}`)}
            </h3>
      ...
      <div style={{
        ...
        >
          {saving ? t('homeDetails.saving') : t('homeDetails.saveButton')}
        </button>
      </div>
    </div>
  );
...
export default HomeDetailsEditor;
